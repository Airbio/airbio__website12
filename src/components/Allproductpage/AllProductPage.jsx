...
const AllProductPage = () => {
    ...
    return (
        <div>
            ...
            <div className='p-5'>
                <div className=' md:w-[80%] mx-auto md:flex justify-between items-center'>
                    <div>
                        <h1 className='text-2xl md:text-5xl font-semibold text-center md:text-start my-3 md:mb-5 text-[#1782C5]  md:w-[250px]'>Airbio Biosafety Cabinets</h1>
                        <div className='flex items-center justify-center gap-3'>
                            <p className='border border-black p-1 px-4 text-sm rounded-3xl'>Precision</p>
                            <p className='border border-black p-1 px-4 text-sm rounded-3xl'>Sterility</p>
                            <p className='border border-black p-1 px-4 text-sm rounded-3xl'>Efficiency</p>
                        </div>
                    </div>
                    <div>
                ...
                <div>
                    <div>
                        <div className="gap-3 flex flex-col my-5 items-center">
                            <h1 className="p-2 px-4 text-2xl mt-8 border border-[#C0C0C0] rounded-full font-semibold inline-block text-center">
                        ...
                        <div>
                            <div className="flex overflow-x-scroll  gap-4 md:gap-10 p-4 md:w-[90%] mx-auto">
                                {cardsData.map((card, index) => (
                                    <div key={index}>
                                        ...
                                        <div className="py-2 flex flex-col justify-center  items-center md:items-start">
                                            <h1 className="font-semibold md:text-start text-lg">{card.subtitle}</h1>
                                            <p className="text-[#1782C5] text-center mb-3 md:text-start h-[50px]">{card.description}</p>
                                            <button className="px-4 py-1 flex justify-center items-center gap-2 rounded-[50px] mt-5 border border-[#CDCDCD]">
                                                {card.buttonText}
                ...
                <div className="my-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:wp-5">
                        {features
                            .filter((_, index) => showAll || index < 6)
                            .map((feature, index) => (
                                <div
                                    key={index}
                                    className="flex w-full max-w-[350px] bg-[#F8F8F8] py-5 rounded-xl mx-auto"
                                >
                                    <div className="pl-3 py-1 px-2">
                                        <img src={feature.icon} alt={feature.title} className="w-11 h-11" />
                                    </div>
                                    <div className="px-4">
                                        <h1 className="font-semibold mb-4">{feature.title}</h1>
                                        <p className="text-sm mb-5 text-[#1E1E1E] text-opacity-75">
                                            {feature.description}
                                        </p>
                                    </div>
                                </div>
                ...
                <div className='my-10'>
                    <div className='bg-[#F8F8F8] md:w-[80%] mx-auto p-5 px-6 flex flex-col md:flex-row items-center gap-5 rounded-lg'>
                        ...
                        <div className='md:w-[60%] md:mr-8 '>
                            <h1 className='text-xl md:text-3xl font-semibold mb-5'>About <span className='text-[#1782C5]'>AirBio Biosafety Cabinets</span> </h1>
                            <p className='text-justify text-[14px] text-opacity-75'><span className='font-semibold'>AirBio Biosafety Cabinets (BSCs)</span> are precision-engineered laboratory workspaces designed for the safe handling of potentially hazardous or contaminated materials. By combining unidirectional airflow from the top with air inflow from the surrounding room, these cabinets create a dynamic air barrier that protects both users and their materials from contamination. <br />They are available in two variants - <span className='font-semibold'>Class II A2</span >and <span className='font-semibold'>Class II B2</span>.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className='my-10'>
                    <div className='bg-[#F8F8F8] md:w-[80%] mx-auto p-5 px-6 flex flex-col md:flex-row items-center gap-5 rounded-lg'>
                        ...
                        <div className='md:w-[60%] md:mr-8 '>
                            <h1 className='text-xl md:text-3xl font-semibold mb-5'>Tailored to Meet <span className='text-[#1782C5]'>Biosafety</span> Needs</h1>

                            <p className='text-justify text-[14px] text-opacity-75'>Biosafety Cabinets come in various classes, each catering to specific bio-containment requirements. Within these classes, the cabinets differ based on how air is recirculated or exhausted. Regardless of the type, all AirBio BSCs leverage <span className='font-semibold'>High Efficiency Particulate Air (HEPA)</span> or <span className='font-semibold'>Ultra Low Particulate Air (ULPA)</span> filters to maintain a sterile workspace.</p>
                        </div>
                    </div>
                </div>
            </div>
